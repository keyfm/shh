...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
	...
	V@: changed only part in header_id to 'locus tag' being printed instead on long header
                                ''',
	...
	for feat in seqIOrecord.features:
		if feat.type == feature:
			...
			if 'pseudo' not in feat.qualifiers:
				header_id = str(feat.qualifiers.get('locus_tag')[0]) #+'_'+feat.qualifiers.get('product','NA')[0].title().replace(" ", "")+'_'+feat.qualifiers.get('protein_id','NA')[0]
