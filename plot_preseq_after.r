...
if( !is.na(args[6]) ){ capres <- read.csv(args[6],check.names=F);rownames(capres) <- capres[,'Sample Name'] } # file contains EAGER results cap3
...
## extract info from capture results
cf.extraSeq <- round(capres[sampleID,'# mapped reads prior RMDup']/capres[sampleID,'Mapped Reads after RMDup'],2)

## plot
maxY <- max(d2[,'UPPER_0.95CI'])+(0.1*max(d2[,'UPPER_0.95CI']))
pdf(paste(outfolder,sampleID,"_preseq_afterExtraSeq.pdf",sep=""))
...
points(capres[sampleID,'# mapped reads prior RMDup'],capres[sampleID,'Mapped Reads after RMDup'] ,pch=8,col='red',cex=2)
## legend
## NOTE: For each CF I plot the amount of distinct reads expected at this CF, and the amount of extra sequencing necessary to get there (considering %endo and already performed sequencing)
...
legend('topleft',legend=paste('actual CF:',cf.extraSeq,'; #Dist:',round(capres[sampleID,'Mapped Reads after RMDup']/1000000,2),'M; #TotalSeq:',round(capres[sampleID,'# of Raw Reads prior Clip & Merge (C&M)']/1000000,2),'M',sep=""),fill="red")
dev.off()
